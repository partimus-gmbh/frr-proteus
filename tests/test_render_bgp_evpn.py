...
def test_vni_block():
    ...
    vni.rd.ipv4.administrator = ipaddress.IPv4Address("10.10.10.10")
...
def test_vrf_rd_and_route_target():
    ...
    evpn = instance.afi_safis.l2vpn_evpn
    evpn.rd.ipv4.administrator = ipaddress.IPv4Address("10.10.10.10")
...
def test_vrf_route_target_export_all_encodings():
    ...
    export.ipv4.append(
        EvpnAf.RouteTargetExport.Ipv4(
            global_admin=ipaddress.IPv4Address("10.10.10.10"), local_admin=100
        )
    )
...
def test_route_target_component_ranges_enforced():
    ...
    with pytest.raises(bindings.YangValidationError):
        # deliberately ill-typed: the runtime check must reject it
        EvpnAf.RouteTargetExport.Ipv4(
            global_admin="999.1.1.1",  # type: ignore[arg-type]
            local_admin=100,
        )
    with pytest.raises(bindings.YangValidationError):
        instance = _new_instance(vrf="vrf-red")
...
def test_autort_and_mac_vrf_soo():
    ...
    evpn.mac_vrf_soo.ipv4.global_admin = ipaddress.IPv4Address("192.0.2.10")
...
def test_default_originate_and_advertise_pip():
    ...
    evpn.advertise_pip.ip = ipaddress.IPv4Address("192.0.2.5")
    text = render_bgp_instance(instance)
...
def test_evpn_type5_network_statement():
    ...
    evpn = instance.afi_safis.l2vpn_evpn
    net = EvpnAf.Network(
        prefix=ipaddress.IPv4Network("203.0.113.0/24"),
        ethtag=0,
        label=4001,
        esi="00:00:00:00:00:00:00:00:00:00",
        gwip=ipaddress.IPv4Address("192.0.2.6"),
