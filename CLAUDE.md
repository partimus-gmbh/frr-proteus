...
- `yang/custom/` -- **the schema new work targets**: a self-contained,
  ...
  carry no zebra config), `proteus-system.yang` (host-level
  lines: `frr defaults`, `hostname`, `log syslog LEVEL`, `service
  integrated-vtysh-config` -- just enough that real-world examples
  don't need a literal preamble), and
  `proteus-configuration-metadata.yang` (RFC 7952 `md:annotation`
  statements, Juniper junos-configuration-metadata style; currently
  just `comment`, type string. Defines NO data nodes -- annotations
  are instance metadata attachable to any node of the sibling
  modules. `ietf-yang-metadata@2016-08-05.yang` is vendored under
  yang/vendor/ietf like the other RFC modules; pyang's bundled
  metadata plugin grammar-checks md:annotation automatically)).
  ASN-valued nodes are STRUCTURED
...
- `src/frr_proteus/_generated/` -- generated bindings (the `frr_bgp/`
  ...
  *not* applied, so templates can rely on "falsy means not explicitly
  ...
  paths). RFC 7952 metadata annotations are generated by DEFAULT
  (opt-out: `--no-dataclass-annotations`): every `md:annotation`
  statement in the compile set lands in a `_YANG_ANNOTATIONS`
  registry, and `annotate(node, comment=...)` attaches metadata to
  the node itself, `annotate(node, "leaf_field", comment=...)` to a
  scalar leaf member (container/list members are annotated on the
  child object, never via the parent), `annotate(node, "ll_field",
  i, comment=...)` to the i-th leaf-list entry (RFC 7952 forbids
  whole-leaf-list annotation); `annotations(...)` reads back, value
  None removes, values are validated against the annotation's type.
  Storage is a per-instance `_yang_metadata` dict invisible to
  `__init__`/`repr`/`==`; serde round-trips per RFC 7952 5.2 (`"@"`
  member inside container/list-entry objects, sibling `"@member"`
  for leaves, null-padded `"@member"` arrays for leaf-list entries,
  annotation names always `module-name:name`-qualified; metadata on
  an unset leaf is dropped at encode). Everything is driven by
  per-class `_yang_fields` ClassVar
